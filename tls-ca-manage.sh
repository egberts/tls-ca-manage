...
function create_generic_ca_req_config_file
{
  ...
  TIMESTAMP="$(date)"
  cat << OPENSSL_CNF_EOF | tee "$IA_OPENSSL_CNF" > /dev/null
#
# File: $IA_OPENSSL_CNF
# Generated by: $0
...
function create_generic_ca_extension_config_file
{
    ...
    # create file, appends later on
    cat << CIC_IN_EOF | tee "$CIC_INTERNODE_CONFIG_FILESPEC" > /dev/null
#
...
CIC_IN_EOF
    ...
    cat << CIC_IN_EOF | tee -a "$CIC_INTERNODE_CONFIG_FILESPEC" > /dev/null
[ crl_ext ]
authorityKeyIdentifier  = keyid:always
authorityInfoAccess     = @issuer_info
...
# TODO: We need to stop using this GENPKEY soon
OPENSSL_GENPKEY="$OPENSSL genpkey"  # only used by chacha20
